...
void ScanProcessMemory(void) {
    ...
    HMODULE hMod = GetModuleHandleA(NULL);  // TargetApp.exe
    if (!hMod) {
    ...
    // Dump Battlenet::Session::Salt from TargetApp.exe
    ...
    fprintf(log, "[*] Scanning TargetApp.exe memory...\n");
    ...
    if (modInfo.SizeOfImage > 0x1000) {
        ScanMemoryRegion((const unsigned char*)modInfo.lpBaseOfDll + 0x1000, modInfo.SizeOfImage - 0x1000, "TargetApp.exe (after PE header)");
    } else {
        ScanMemoryRegion((const unsigned char*)modInfo.lpBaseOfDll, modInfo.SizeOfImage, "TargetApp.exe");
    }
